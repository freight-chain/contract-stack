...
contract("SFC", async ([firstStaker, secondStaker, thirdStaker, firstDepositor, secondDepositor, thirdDepositor]) => {
  ...
  describe("Locking stake tests", async () => {
    it('should start "locked stake" feature', async () => {
      ...
      await expectRevert(
        this.stakers.startLockedUp(currentEpoch, { from: other_address }),
      ...
      await expectRevert(
        this.stakers.startLockedUp(currentEpoch.sub(new BN("1")), { from: sfc_owner }),
    ...
    it("should calc raw ValidatorEpochReward correctly after locked up started", async () => {
      ...
      let epoch = new BN("1");
      expect(await this.stakers.calcRawValidatorEpochReward(firstStakerID, epoch)).to.be.bignumber.equal(
        ether("0.000000941176470588"),
      );
      expect(await this.stakers.calcRawValidatorEpochReward(secondStakerID, epoch)).to.be.bignumber.equal(
        ether("0.000000058823529411"),
      );
      expect(await this.stakers.calcRawValidatorEpochReward(thirdStakerID, epoch)).to.be.bignumber.equal(ether("0"));

      epoch = new BN("2");
      expect(await this.stakers.calcRawValidatorEpochReward(firstStakerID, epoch)).to.be.bignumber.equal(
        ether("0.000000842105263157"),
      );
      expect(await this.stakers.calcRawValidatorEpochReward(secondStakerID, epoch)).to.be.bignumber.equal(
        ether("0.000000052631578947"),
      );
      expect(await this.stakers.calcRawValidatorEpochReward(thirdStakerID, epoch)).to.be.bignumber.equal(
        ether("0.000000105263157894"),
      );
    ...
    it("should lock stake", async () => {
      ...
      let epoch = new BN("1");
      expect(
        await this.stakers.calcValidatorEpochReward(firstStakerID, epoch, this.validatorComission),
      ).to.be.bignumber.equal(ether("0.000000191176470588"));
      expect(
        await this.stakers.calcValidatorEpochReward(secondStakerID, epoch, this.validatorComission),
      ).to.be.bignumber.equal(ether("0.000000058823529411"));
      expect(
        await this.stakers.calcValidatorEpochReward(thirdStakerID, epoch, this.validatorComission),
      ).to.be.bignumber.equal(ether("0"));

      epoch = new BN("2");
      expect(
        await this.stakers.calcValidatorEpochReward(firstStakerID, epoch, this.validatorComission),
      ).to.be.bignumber.equal(ether("0.000000171052631578"));
      expect(
        await this.stakers.calcValidatorEpochReward(secondStakerID, epoch, this.validatorComission),
      ).to.be.bignumber.equal(ether("0.000000052631578947"));
      expect(
        await this.stakers.calcValidatorEpochReward(thirdStakerID, epoch, this.validatorComission),
      ).to.be.bignumber.equal(ether("0.000000105263157894"));
      ...
      expect(await this.stakers.calcValidatorEpochReward(firstStakerID, epoch, this.validatorComission)).to.be.bignumber.equal(ether('0.000000401315789473'));
    ...
    it("should lock stake with right duration", async () => {
      ...
      await expectRevert(
        this.stakers.lockUpStake(minDuration.sub(new BN("1")), { from: firstStaker }),
        "incorrect duration",
      );
      await this.stakers.lockUpStake(minDuration, { from: firstStaker });
      await expectRevert(
        this.stakers.lockUpStake(maxDuration.add(new BN("1")), { from: secondStaker }),
        "incorrect duration",
      );
    ...
    it("should not call prepareToWithdrawStakePartial, until locked time is pass", async () => {
      ...
      const wrID = new BN("1");
      await expectRevert(
        this.stakers.prepareToWithdrawStakePartial(wrID, ether("1.0"), { from: firstStaker }),
        "stake is locked",
      );
      time.increase(86400 * 14 - 2);
      await expectRevert(
    ...
    it("should lock delegation", async () => {
      ...
      expect(
        await this.stakers.calcDelegationEpochReward(
          firstDepositor,
          firstStakerID,
          epoch,
          ether("5.0"),
          this.validatorComission,
        ),
      ).to.be.bignumber.equal(ether("0.000000249999999999"));
      expect(
        await this.stakers.calcDelegationEpochReward(
          thirdDepositor,
          firstStakerID,
      ...
      ).to.be.bignumber.equal(ether("0.000000499999999999"));
      ...
      ).to.be.bignumber.equal(ether("0.000000223684210526"));
      expect(
        await this.stakers.calcDelegationEpochReward(
          thirdDepositor,
          firstStakerID,
      ...
      ).to.be.bignumber.equal(ether("0.000000447368421052"));
      ...
      await expectRevert(
        this.stakers.lockUpDelegation(duration, firstStakerID, { from: firstDepositor }),
        "staker's locking will finish first",
      );
      await this.stakers.lockUpStake(duration.add(new BN("10")), { from: firstStaker });
      ...
      /*expect(await this.stakers.calcValidatorEpochReward(firstStakerID, epoch, this.validatorComission)).to.be.bignumber.equal(ether('0.000000167982456139'));
      expect(await this.stakers.calcDelegationEpochReward(firstDepositor, firstStakerID, epoch, ether('5.0'), this.validatorComission)).to.be.bignumber.equal(ether('0.000000650438596490'));
      // reduce unlock stake by 70%
      expect(await this.stakers.calcValidatorEpochReward(secondStakerID, epoch, this.validatorComission)).to.be.bignumber.equal(ether('0.000000015789473684'));
      expect(await this.stakers.calcValidatorEpochReward(thirdStakerID, epoch, this.validatorComission)).to.be.bignumber.equal(ether('0.000000031578947368'));
      expect(await this.stakers.calcDelegationEpochReward(thirdDepositor, firstStakerID, epoch, ether('10.0'), this.validatorComission)).to.be.bignumber.equal(ether('0.000000134210526315'));
      ...
      expect(await this.stakers.calcValidatorEpochReward(firstStakerID, epoch, this.validatorComission)).to.be.bignumber.equal(ether('0.000000092492260061'));
      expect(await this.stakers.calcValidatorEpochReward(secondStakerID, epoch, this.validatorComission)).to.be.bignumber.equal(ether('0.000000056965944272'));
      expect(await this.stakers.calcDelegationEpochReward(firstDepositor, firstStakerID, epoch, ether('5.0'), this.validatorComission)).to.be.bignumber.equal(ether('0.000000272987616098'));
      expect(await this.stakers.calcDelegationEpochReward(thirdDepositor, firstStakerID, epoch, ether('10.0'), this.validatorComission)).to.be.bignumber.equal(ether('0.000000545975232197'));
      // reduce unlock stake by 70%
      expect(await this.stakers.calcValidatorEpochReward(thirdStakerID, epoch, this.validatorComission)).to.be.bignumber.equal(ether('0.000000031578947368'));
      ...
      expect(await this.stakers.calcValidatorEpochReward(firstStakerID, epoch, this.validatorComission)).to.be.bignumber.equal(ether('0.000000109649122806'));
      expect(await this.stakers.calcValidatorEpochReward(secondStakerID, epoch, this.validatorComission)).to.be.bignumber.equal(ether('0.000000074122807017'));
      expect(await this.stakers.calcDelegationEpochReward(thirdDepositor, firstStakerID, epoch, ether('10.0'), this.validatorComission)).to.be.bignumber.equal(ether('0.000000717543859648'));
      // reduce unlock stake by 70%
      expect(await this.stakers.calcDelegationEpochReward(firstDepositor, firstStakerID, epoch, ether('5.0'), this.validatorComission)).to.be.bignumber.equal(ether('0.000000067105263157'));
      expect(await this.stakers.calcValidatorEpochReward(thirdStakerID, epoch, this.validatorComission)).to.be.bignumber.equal(ether('0.000000031578947368'));

      time.increase(20);
      await this.stakers.makeEpochSnapshots(); // epoch #10
      ...
      time.increase(10000);
      epoch = new BN('12');
      await this.stakers.makeEpochSnapshots(10000, false); // epoch #12
      // reduce unlock stake by 70%
      expect(await this.stakers.calcDelegationEpochReward(firstDepositor, firstStakerID, epoch, ether('5.0'), this.validatorComission)).to.be.bignumber.equal(ether('0.000000067105263157'));
      expect(await this.stakers.calcDelegationEpochReward(thirdDepositor, firstStakerID, epoch, ether('10.0'), this.validatorComission)).to.be.bignumber.equal(ether('0.000000134210526315'));
      expect(await this.stakers.calcValidatorEpochReward(firstStakerID, epoch, this.validatorComission)).to.be.bignumber.equal(ether('0.000000051315789473'));
      ...
      epoch = new BN('13');
      await this.stakers.makeEpochSnapshots(10000, false); // epoch #13
      // reduce unlock stake by 70%
      expect(await this.stakers.calcDelegationEpochReward(firstDepositor, firstStakerID, epoch, ether('5.0'), this.validatorComission)).to.be.bignumber.equal(ether('0.000000067105263157'));
      expect(await this.stakers.calcDelegationEpochReward(thirdDepositor, firstStakerID, epoch, ether('10.0'), this.validatorComission)).to.be.bignumber.equal(ether('0.000000134210526315'));
      expect(await this.stakers.calcValidatorEpochReward(firstStakerID, epoch, this.validatorComission)).to.be.bignumber.equal(ether('0.000000051315789473'));
    ...
    it("should lock delegation with right duration", async () => {
      ...
      await expectRevert(
        this.stakers.lockUpDelegation(minDuration.sub(new BN("1")), secondStakerID, { from: firstDepositor }),
        "incorrect duration",
      );
      await this.stakers.lockUpDelegation(minDuration, secondStakerID, { from: firstDepositor });
      await expectRevert(
        this.stakers.lockUpDelegation(maxDuration.add(new BN("1")), firstStakerID, { from: secondDepositor }),
        "incorrect duration",
      );
      await this.stakers.lockUpDelegation(maxDuration.sub(new BN("1")), firstStakerID, { from: secondDepositor });
      await expectRevert(
        this.stakers.lockUpDelegation(minDuration, firstStakerID, { from: secondDepositor }),
        "already locked up",
      );
      await expectRevert(
        this.stakers.lockUpDelegation(minDuration.mul(new BN("3")), secondStakerID, { from: firstDepositor }),
        "staker's locking will finish first",
      );
