...
contract("SFC", async ([firstStaker, secondStaker, thirdStaker, firstDepositor, secondDepositor, thirdDepositor]) => {
  ...
  describe("Methods tests", async () => {
    it("checking Staker parameters", async () => {
      ...
      expect(await this.stakers.delegationLockPeriodTime.call()).to.be.bignumber.equal(
        new BN("86400").mul(new BN("7")),
      );
    ...
    it("checking createDelegation function", async () => {
      ...
      await expectRevert(
        this.stakers.createDelegation(secondStakerID, {
      ...
      await expectRevert(
        this.stakers.createDelegation(zeroStakerID, {
          from: secondDepositor,
          value: ether("1.0"),
        }),
        "staker doesn't exist",
      ...
      await expectRevert(
        this.stakers.createDelegation(firstStakerID, {
          from: secondDepositor,
          value: ether("0.99"),
        }),
        "insufficient amount",
    ...
    it("checking calcRawValidatorEpochReward function", async () => {
      expect(await this.stakers.calcRawValidatorEpochReward(new BN("1"), new BN("1"))).to.be.bignumber.equal(
        ether("0.0"),
      );
      ...
      expect(await this.stakers.calcRawValidatorEpochReward(firstStakerID, new BN("1"))).to.be.bignumber.equal(
        ether("1.333333333333333263"),
      );
      expect(await this.stakers.calcRawValidatorEpochReward(secondStakerID, new BN("1"))).to.be.bignumber.equal(
        ether("0.166666666666666735"),
      );
    });
    ...
    it("checking calcValidatorEpochReward function", async () => {
      ...
      expect(
        await this.stakers.calcValidatorEpochReward(firstStakerID, new BN("1"), this.validatorComission),
      ).to.be.bignumber.equal(ether("0.267647249999999983"));
      expect(
        await this.stakers.calcValidatorEpochReward(secondStakerID, new BN("1"), this.validatorComission),
      ).to.be.bignumber.equal(ether("0.082353000000000076"));
      expect(
        await this.stakers.calcValidatorEpochReward(thirdStakerID, new BN("1"), this.validatorComission),
    ...
    it("checking calcDelegationEpochReward function", async () => {
      ...
      ).to.be.bignumber.equal(ether("0.350000249999999979"));
    });
    ...
    it("checking claimDelegationRewards function", async () => {
      ...
      await expectRevert(
        this.stakers.claimDelegationRewards(new BN("1") /*, firstStakerID*/, { from: firstStaker }),
        "delegation doesn't exist",
      );
      await expectRevert(
        this.stakers.claimDelegationRewards(new BN("0") /*, firstStakerID*/, { from: firstDepositor }),
        "no epochs claimed",
      );
      ...
      await expectRevert(
        this.stakers.claimDelegationRewards(new BN("1") /*, firstStakerID*/, { from: firstDepositor }),
        "future epoch",
      );
      ...
      await expectRevert(
        this.stakers.claimDelegationRewards(new BN("100") /*, firstStakerID*/, { from: firstDepositor }),
        "delegation is deactivated",
      );
    });
    ...
    it("checking claimValidatorRewards function", async () => {
      await expectRevert(
        this.stakers.claimValidatorRewards(new BN("1"), { from: firstStaker }),
        "staker doesn't exist",
      );
      await this.stakers._createStake({ from: firstStaker, value: ether("1.0") });
      let firstStakerID = await this.stakers.getStakerID(firstStaker);
    ...
    it("checking prepareToWithdrawDelegation function", async () => {
      ...
      let firstStakerID = await this.stakers.getStakerID(firstStaker);
      await expectRevert(
        this.stakers.prepareToWithdrawDelegation(/*firstStakerID,*/ { from: firstDepositor }),
        "delegation doesn't exist",
      );
    ...
    it("checking withdrawDelegation function", async () => {
      ...
      await expectRevert(
        this.stakers.withdrawDelegation(/*firstStakerID,*/ { from: firstDepositor }),
        "delegation wasn't deactivated",
      );
      await expectRevert(
        this.stakers.prepareToWithdrawDelegation(/*firstStakerID,*/ { from: firstDepositor }),
        "delegation doesn't exist",
      ...
      await expectRevert(
        this.stakers.prepareToWithdrawDelegation(/*firstStakerID,*/ { from: thirdDepositor }),
      ...
      await expectRevert(
        this.stakers.withdrawDelegation(/*firstStakerID,*/ { from: thirdDepositor }),
        "not enough time passed",
      );
