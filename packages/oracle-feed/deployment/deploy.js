// FreightTrust Voting smart contract deployment script
const fs = require("fs");
 ...
 * @returns {Promise<{}>}
 */
async function deploy(client, abiFile, byteCodeFile, expirationPeriod, feeds) {
  let abi, byteCode;
  ...
  return contract
    .deploy({
      data: byteCode,
      arguments: [client.utils.numberToHex(expirationPeriod), feeds],
    })
    .send({
      from: srcAddress,
...
deploy(client, abiFilePath, byteCodeFilePath, 1800, [srcAddress])
  .then(res => {
    // log the success
    console.log(res, "\nDeployed.\n");
    return 0;
