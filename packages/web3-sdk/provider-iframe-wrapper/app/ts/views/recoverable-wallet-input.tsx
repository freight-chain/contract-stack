...
export const RecoverableWalletInput = (model: RecoverableWalletInputModel) => {
	...
	const [queuedAddressWalletGeneration, setQueuedAddressWalletGeneration] = React.useState<
		[bigint | undefined, boolean, boolean] | undefined
	>(undefined);
	const recoverableWalletAddressStringChanged = async (
		...
		try {
			...
			if (underlyingWallet === undefined)
				throw new Error(`Cannot setup a recoverable wallet without an underlying wallet.`);
			const recoverableWallet =
				underlyingWallet instanceof ViewingWallet || underlyingWallet instanceof ViewingRecoverableWallet
			...
			if (ownerAddress !== underlyingWallet.address)
				throw new Error(
					`The currently selected signing wallet (${underlyingWallet.address
						.toString(16)
						.padStart(40, "0")}) is not the owner of this wallet, ${ownerAddress.toString(16).padStart(40, "0")} is.`,
				);
			...
			{underlyingWallet === undefined && (
				<>
					...
					<WalletSelector
						...
						fetch={model.fetch}
						jsonRpcEndpoint={model.jsonRpcEndpoint}
						getGasPrice={model.getGasPrice}
						walletChanged={setUnderlyingWallet}
					/>
				</>
			)}
			{underlyingWallet !== undefined && (
				<AddressInput
					placeholder="Recoverable Wallet Address"
					errorHandler={model.errorHandler}
					onChange={model.errorHandler.asyncWrapper(recoverableWalletAddressStringChanged)}
				/>
			)}
			{underlyingWallet !== undefined && wallet !== undefined && (
