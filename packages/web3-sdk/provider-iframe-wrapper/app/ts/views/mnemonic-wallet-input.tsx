...
export const MnemonicWalletInput = (model: MnemonicWalletInputModel) => {
	...
	const [queuedMnemonicWalletGeneration, setQueuedMnemonicWalletGeneration] = React.useState<string[] | undefined>(
		undefined,
	);
	...
	const mnemonicWordsChanged = async (words: string[]) => {
		...
		try {
			...
			const wallet =
				mnemonicError === null
					? await MnemonicWallet.create(model.jsonRpcEndpoint, model.fetch, model.getGasPrice, words)
					: undefined;
			setWallet(wallet);
		} finally {
			...
			{!generatingRandomMnemonic && (
				<>
					...
					<input
						...
						value={mnemonicWords}
						onChange={model.errorHandler.asyncWrapper(inputChanged)}
					/>
					{mnemonicError !== undefined && <div className="mnemonic-error">{mnemonicError}</div>}
					{wallet !== undefined && (
						<div>
							<label>Address: </label>
							<label className="monospace">{wallet.address.toString(16)}</label>
						</div>
					)}
