...
export async function constructorDataBytes(
	constructorSignature: string,
	constructorParameters: ReadonlyArray<Encodable>,
	deploymentBytecode: Iterable<number>,
): Promise<Uint8Array> {
...
export function contractParametersToEncodables(
	contractParameters: shared.ContractParameterArray,
): ReadonlyArray<Encodable> {
...
function contractParameterToEncodable(contractParameter: shared.ContractParameter): Encodable {
	if (Array.isArray(contractParameter)) {
		...
		typeof contractParameter === "object" &&
		!(contractParameter instanceof Uint8Array) &&
		!Array.isArray(contractParameter)
	) {
