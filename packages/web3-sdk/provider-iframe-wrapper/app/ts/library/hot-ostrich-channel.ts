...
export class HotOstrichChannel implements provider.HotOstrichHandler {
	...
	public readonly legacyJsonrpc: provider.HotOstrichHandler["legacyJsonrpc"] = async (method, parameters) => {
		switch (method) {
			case "eth_gasPrice":
				return await bigintToAddressString(await this.getGasPrice());
			...
			case "eth_uninstallFilter": {
				return await this.jsonRpc.remoteProcedureCall({
					id: 1,
					jsonrpc: "2.0",
			...
			default: {
				if (this.wallet === undefined) {
					switch (method) {
						...
						default:
							throw new JsonRpcError(-32601, `Unknown JSONRPC method: ${method}.`);
					}
				} else {
					switch (method) {
						case "eth_coinbase":
							return await bigintToAddressString(this.wallet.address);
						case "eth_accounts":
							return [await bigintToAddressString(this.wallet.address)];
