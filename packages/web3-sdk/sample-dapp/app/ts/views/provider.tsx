...
export const Provider = (model: ProviderModel) => (
	<div className="provider-container">
		<select
			value={(model.selectedProvider && model.selectedProvider.id) || ""}
			onChange={event => model.onProviderSelected(event.target.value)}
		>
		...
		{model.selectedProvider.wallet !== undefined && (
			<MyAccount
				errorHandler={model.errorHandler}
				getEthBalance={model.selectedProvider.wallet.getEthBalance}
				getTokenBalance={model.selectedProvider.wallet.getTokenBalance}
				address={model.selectedProvider.wallet.address}
				tokens={model.tokens}
			/>
		)}
		{model.executors !== undefined && (
			<AssetManager
				errorHandler={model.errorHandler}
				onSendEth={model.executors.onSendEth}
				onSendToken={model.executors.onSendToken}
				tokens={model.tokens}
			/>
		)}
	</div>
