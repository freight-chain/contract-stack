...
export class HotOstrichChannel implements provider.HotOstrichHandler {
	...
	public readonly updateWallet = (wallet: Wallet | undefined) => {
		...
		this.hotOstrichChannel.updateCapabilities({
			signMessage: canSignMessage,
			signTransaction: canSignTransaction,
	...
	private jsonRpc: JsonRpc = new FetchJsonRpc(this.jsonRpcEndpoint, this.fetch, {
		gasPriceInAttoethProvider: this.getGasPrice,
	});
	...
	public readonly signMessage: provider.HotOstrichHandler["signMessage"] = async message => {
		...
		if (!("signMessage" in this.wallet))
			throw new Error(`Cannot sign a message with this type of wallet.  Are you using a view-only wallet?`);
	...
	public readonly submitContractCall: provider.HotOstrichHandler["submitContractCall"] = async request => {
		...
		if (!("submitContractCall" in this.wallet))
			throw new Error(`Cannot submit transactions with this type of wallet.  Are you using a view-only wallet?`);
	...
	public readonly submitNativeTokenTransfer: provider.HotOstrichHandler["submitNativeTokenTransfer"] = async request => {
		...
		if (!("submitContractCall" in this.wallet))
			throw new Error(`Cannot transfer ETH with this type of wallet.  Are you using a view-only wallet?`);
	...
	public readonly legacyJsonrpc: provider.HotOstrichHandler["legacyJsonrpc"] = async (method, parameters) => {
		switch (method) {
			...
			case "eth_coinbase": {
				return this.wallet === undefined ? null : bigintToHexString(this.wallet.address);
			}
			case "eth_accounts": {
				return this.wallet === undefined ? [] : [bigintToHexString(this.wallet.address)];
			}
			case "eth_estimateGas":
			case "eth_call": {
				return this.wallet === undefined
					? (
							await this.jsonRpc.remoteProcedureCall({
								id: 1,
								jsonrpc: "2.0",
