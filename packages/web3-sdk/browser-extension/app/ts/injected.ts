...
const providers: Record<
	string,
	shared.Handshake.ProviderAnnouncementNotification["payload"] & { hotOstrich: client.HotOstrichChannel }
> = {};
const getRandomLegacyProvider = () =>
	Object.values(providers).length === 0
		? undefined
		: Object.values(providers).find(provider => provider.hotOstrich.capabilities.has("legacy"));
		...
		const hotOstrich = new client.HotOstrichChannel(window, announcement.provider_id, {
			...
			onWalletAddressChanged: () => {
				for (const callback of accountChangedSubscribers) {
					callback(
						hotOstrich.walletAddress === undefined
							? []
							: [`0x${hotOstrich.walletAddress.toString(16).padStart(40, "0")}`],
					);
				}
			},
		});
...
const sendAsync = async (
	payload: { readonly method: string; readonly params: unknown[] },
	callback: (
		error: { readonly code: number; readonly message: string; readonly data?: unknown } | null,
		result: unknown | null,
	) => void,
) => {
