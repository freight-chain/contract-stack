...
import {
	...
	IOffChainTransaction,
	RawOffChainTransaction,
} from "@zoltu/ethereum-types";
...
export class ViewingWallet {
	...
	public constructor(
		...
		this.jsonRpc = new FetchJsonRpc(jsonRpcEndpoint, fetch, {
			gasPriceInAttoethProvider: getGasPrice,
			addressProvider: async () => address,
		});
	}
	...
	public readonly legacyJsonrpc: (
		method: "eth_call" | "eth_estimateGas" | "eth_sendTransaction" | "eth_signTransaction",
		parameters: unknown[],
	) => unknown = async (method, parameters) => {
		if (method === "eth_sendTransaction" || method === "eth_signTransaction")
			throw new JsonRpcError(-32601, `${method} is not supported by this wallet.`);
		...
		switch (method) {
			...
			case "eth_signTransaction": {
				...
				const unsignedTransaction = {
					...
					nonce: BigInt(
						transaction.nonce || (await this.jsonRpc.getTransactionCount(gasEstimatingTransaction.from, "pending")),
					),
...
export class LedgerWallet {
	...
	public readonly signMessage: provider.HotOstrichHandler["signMessage"] = async message => {
		...
		if (signature.v !== 27n && signature.v !== 28n)
			throw new Error(
				`Ledger returned a signature with a 'v' value of ${signature.v}.  The only valid values for 'v' are 27 or 28.`,
			);
		return {
			requested_message: message,
			signed_message: stringToHash,
...
export class ViewingRecoverableWallet {
	public constructor(public readonly underlyingWallet: ViewOnlyWallet, public readonly address: bigint) {}
	...
	public readonly legacyJsonrpc: (
		...
		const mutatedTransaction = {
			...originalTransaction,
			to: `0x${this.address.toString(16).padStart(40, "0")}`,
			data: data.to0xString(),
			value: `0x0`,
		};
...
export class RecoverableWallet {
	public constructor(public readonly underlyingWallet: SigningWallet, public readonly address: bigint) {}
