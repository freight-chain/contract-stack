...
async function onContentScriptConnected(port: browser.runtime.Port): Promise<void> {
	...
	const removeEventListener = (_: "message", listener: (payload: any) => void) =>
	...
	const hotOstrichChannel = new HotOstrichChannel(
		errorHandler,
		fetch.bind(window),
		{ addEventListener, removeEventListener },
		{ postMessage },
	...
	const mnemonicWallet = await MnemonicWallet.create(jsonRpcEndpoint, fetch.bind(window), getGasPrice, [
		"zoo",
		"zoo",
		"zoo",
		"zoo",
