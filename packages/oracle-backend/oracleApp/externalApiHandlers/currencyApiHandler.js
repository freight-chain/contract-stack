...
function isAvailableMethod(methodName) {
  for (var i = 0; i < availableApiMethods.length; i++) {
    if (methodName == availableApiMethods[i]) return true;
  }
  return false;
}

function validateJsonDescription(jsonDescription) {
  if (!jsonDescription) throwCannotBeEmpty("");

  if (!jsonDescription.name || jsonDescription.name == "") throwCannotBeEmpty("name");

  if (!jsonDescription.path || jsonDescription.path == "") throwCannotBeEmpty("path");
  ...
  if (!jsonDescription.requestPattern) throwCannotBeEmpty("requestPattern");

  if (!jsonDescription.requestPattern.reqType) throwCannotBeEmpty("requestPattern reqType");
  ...
  if (!jsonDescription.requestPattern.endpointPattern) throwCannotBeEmpty("requestPattern endpointPattern");
}
...
function validatePriceRequest(currency1, currency2) {
  ...
  if (firstIdx < 0)
    throw new Error(
      `currency ${currency1} is not available for request.\nPairs available for request: ${currencyPairs}`,
    );

  let secondIdx = currencyPairs.indexOf(currency2);
  if (secondIdx < 0)
    throw new Error(
      `currency ${currency2} is not available for request.\nPairs available for request: ${currencyPairs}`,
    );
}

function normalizeUri(uri) {
  const httpsPrefix = "https://";
  if (uri.startsWith(httpsPrefix)) return uri;
