...
class IntegrationTests {
  ...
  async createAccounts(passwd, value) {
    for (let i = 0; i < this.accCount; i++) {
      ...
      let chainId = await this.web3.eth.net.getId((err, res) => {
        if (err) console.log("getId ERROR:", err);
      });
      let nonce = await this.web3.eth.getTransactionCount(this.coinbase.address, (err, res) => {
        if (err) console.log("getTransactionCount ERROR:", err);
      });
  ...
  async deployContract() {
    ...
    fs.writeFile(
      `accounts-configs/contract.json`,
      `{"contractAddress":"${newOracleContract.options.address.toLowerCase()}"}`,
      function (err) {
        if (err) {
          console.log(err);
        }
