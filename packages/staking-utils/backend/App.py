...
from modules.FreightTrustApi import FreightTrustApi
...
class App:
    ...
    def run():
        database = Database()
        freighttrustApi = FreightTrustApi()
        ethereumApi = EthereumApi()
        binanceApi = BinanceApi()

        sfcContract = StakersContract(freighttrustApi=freighttrustApi)
        stakerInfoContract = StakerInfoContract(freighttrustApi=freighttrustApi)
        ...
        Events(freighttrustApi=freighttrustApi, database=database).sync()

        # Sync blocks
        print("Syncing blocks ...")
        Blocks(
            sfcContract=sfcContract, freighttrustApi=freighttrustApi, database=database
        ).sync()

        # Sync transactions
        print("Syncing transactions ...")
        Transactions(freighttrustApi=freighttrustApi, database=database).sync()
        ...
        validators = Validators(
            ...
            stakerInfoContract=stakerInfoContract,
            database=database,
        )
        ...
        # Calculate totals
        totalSelfStakedSum = sum(
            validator["selfStakedAmount"] for validator in validators.getAll()
        )
        totalDelegatedSum = sum(
            validator["delegatedAmount"] for validator in validators.getAll()
        )
        totalInUndelegationSum = sum(
            validator["inUndelegationAmount"] for validator in validators.getAll()
        )
        ...
        roi = sfcContract.getRoi(
            totalStaked=totalSelfStakedSum, totalDelegated=totalDelegatedSum
        )
