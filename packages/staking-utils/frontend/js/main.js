function addValidator(validator) {
  ...
  <div class="cell name" data-title="Name">
    ...
    `
      ...
      `
        ...
        ${
          ...
          ${
            validator.logoUrl
              ? `<img class="float-lg-left" src="${validator.logoUrl}" alt="${validator.name} Logo">`
              : `<p class="float-lg-left icon"></p>`
          }
        `
            : `
    ...
    <div class="float-lg-left ml-lg-3 mt-sm-2 mt-lg-0">
      <p class="text-lg-left ${validator.name ? "font-weight-bold" : ""} name"><span>${
    validator.name ? validator.name : "unknown"
  }${
    validator.name && validator.isVerified
      ? ` <i class="fas fa-check-circle verified" title="Verified via Blockchain"></i>`
      : ""
  }</span></p>
      <p class="text-lg-left font-weight-light text-truncate address"><a href="https://explorer.freighttrust.network/validator/${
        validator.address
      }" target="_blank">${validator.address.toLowerCase()}</a></p>
    </div>
  </div>
  ...
  <div class="cell text-light" data-title="In Undelegation"><p>${numeral(validator.inUndelegationAmount).format(
    "0,0",
  )} FTM</p></div>
  <div class="cell" data-title="Total Staked"><p>${numeral(validator.totalStakedAmount).format("0,0")} FTM</p></div>
  <div class="cell" data-title="Available Capacity"><p>${numeral(validator.availableCapacityAmount).format(
    "0,0",
  )} FTM</p></div>
  <div class="cell text-light" data-title="Staking Power"><p>${numeral(validator.stakingPowerPercent).format(
    "0.00%",
  )}</p></div>
...
function updateGeneral() {
  axios.get("https://freighttruststaker.info/api/v1/general").then(response => {
    const general = response.data;

    document.querySelector("#total-self-staked-sum").innerText =
      numeral(general.totalSelfStakedSum).format("0,0") + " FTM";
    document.querySelector("#total-self-staked-percent").innerText = numeral(general.totalSelfStakedPercent).format(
      "0.00%",
    );
    document.querySelector("#total-delegated-sum").innerText =
      numeral(general.totalDelegatedSum).format("0,0") + " FTM";
    document.querySelector("#total-delegated-percent").innerText = numeral(general.totalDelegatedPercent).format(
      "0.00%",
    );
    document.querySelector("#total-in-undelegation-sum").innerText =
      numeral(general.totalInUndelegationSum).format("0,0") + " FTM";
    document.querySelector("#total-in-undelegation-percent").innerText = numeral(
      general.totalInUndelegationPercent,
    ).format("0.00%");
    ...
    document.querySelector("#last-updated").innerText = lastUpdated.toLocaleString([], {
      dateStyle: "short",
      timeStyle: "short",
    });
  });
...
function updateValidators() {
  ...
  axios
    .get(
      "https://freighttruststaker.info/api/v1/validators?hideUnknown=" +
        hideUnknown +
        "&sortKey=" +
        sortBy.sortKey +
        "&order=" +
...
(function ($) {
  ...
  document.querySelector(
    `div .row.header .cell [sort-key='${sortBy.sortKey}'] span`,
  ).innerHTML = `<i class='fas fa-sort-${sortBy.order == "desc" ? "down" : "up"}'></i>`;
